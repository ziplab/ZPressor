...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.test_cfg.dec_chunk_size == -1:
            ...
            with self.benchmarker.time("decoder", num_calls=v):
                output = self.run_decoder_in_chunk(
                    ...
                    stable_camera=self.test_cfg.stablize_camera,
                )
        ...
        if self.test_cfg.save_video:
            ...
            save_video(
                [a for a in rgb_gt],
                path / "video_gt" / f"{scene}_gt_frame_{frame_str}.mp4",
            )
        ...
        if self.test_cfg.compute_scores:
            ...
            # Write scene and lpips to txt file, create path recursively if it doesn't exist
