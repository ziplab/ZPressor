        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                try:
                    ...
                    # Skip if the maximum index in context_indices and target_indices exceeds the length of example["images"]
                    if context_indices.max() >= len(example["images"]) or target_indices.max() >= len(example["images"]):
                        continue
                except ValueError:
                ...
                for index in context_indices:
                    idx = index.item()
                    if idx < len(example["images"]):
                        context_images.append(example["images"][idx])
                    else:
                        continue
                ...
                for index in target_indices:
                    idx = index.item()
                    if idx < len(example["images"]):
                        target_images.append(example["images"][idx])
                    else:
                        continue
